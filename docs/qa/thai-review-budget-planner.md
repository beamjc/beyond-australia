...
| BSP-074 ⚠️ | :562 | Label in the coloured box at the bottom of each card, above `{rate}%` | (a) Approval rate for Thai applicants aged {ageGroup} · (b) Approval rate for Thai applicants, all ages — sub-line: {sector}, applying from {location} · Home Affairs, {period} · not a prediction for you | เปอร์เซนต์ที่วีซ่าจะผ่าน | **Owner approved (2026-09-24): show the official rate for the visitor's age group.** (a) อัตราอนุมัติวีซ่าของผู้สมัครชาวไทยอายุ {ageGroup} ปี · (b) อัตราอนุมัติวีซ่าเฉลี่ยของผู้สมัครชาวไทย (ทุกช่วงอายุ) · Sub-line (new, 10 px): ข้อมูลกรมตรวจคนเข้าเมืองออสเตรเลีย {period} · ไม่ใช่การทำนายผลของคุณ | Code picks (a) when `ageSpecific` is true, else (b). `{ageGroup}` e.g. 25–29; `{period}` = ก.ย. 2568 – ส.ค. 2569. Label may wrap to 2 lines in half-card width (~160 px mobile). Also fixes the misspelling เปอร์เซนต์. |
