...
| BSP-061a | CalculationEngine.ts `he-avg` | **Update 2026-09-25 (owner request):** the three university tiers were merged into one card per degree. The card title now reuses the existing goal labels ปริญญาตรี / ปริญญาโท, so only the three VET names in BSP-061 still need Thai. | — | — | — | — |
| BSP-130 | BudgetStudyPlanner.tsx (university card, amber note) | Note under the amount on the single university card | Tuition is an estimate only. The actual fee depends on the faculty and university the student chooses. | ราคาค่าเรียนเป็นการประมาณเท่านั้น ค่าเรียนจะขึ้นอยู่คณะและมหาลัยที่นักเรียนเลือกเรียน *(owner wording, applied verbatim)* | ค่าเรียนนี้เป็นตัวเลขประมาณเท่านั้น ค่าเรียนจริงขึ้นอยู่กับคณะและมหาวิทยาลัยที่เลือก | Two short lines on mobile. Suggested edit adds "กับ" and uses the formal "มหาวิทยาลัย". |
| BSP-131 | BudgetStudyPlanner.tsx (university card) | Optional extra line showing the averaged figure (not applied) | Average tuition about {amount}/year (common range {low} – {high}/year) | — | ค่าเรียนเฉลี่ยประมาณ {amount}/ปี (ส่วนใหญ่อยู่ระหว่าง {low} – {high}/ปี) | Not live. Apply only if the editor and owner want the average shown explicitly. |
...
| BSP-074 ⚠️ | :562 | Label in the coloured box at the bottom of each card, above `{rate}%` | (a) Approval rate for Thai applicants aged {ageGroup} · (b) Estimated approval rate for Thai applicants aged {ageGroup} (few recent cases) — sub-line: {sector}, applying from {location} · Home Affairs, {period} · not a prediction for you | เปอร์เซนต์ที่วีซ่าจะผ่าน | **Owner approved (2026-09-24): show the official rate for the visitor's age group.** (a) อัตราอนุมัติวีซ่าของผู้สมัครชาวไทยอายุ {ageGroup} ปี · (b) อัตราอนุมัติวีซ่าโดยประมาณของผู้สมัครชาวไทยอายุ {ageGroup} ปี (ข้อมูลช่วงอายุนี้มีน้อย) · Sub-line (new, 10 px): ข้อมูลกรมตรวจคนเข้าเมืองออสเตรเลีย {period} · ไม่ใช่การทำนายผลของคุณ | Code picks (a) when `ageSpecific` is true, else (b). `{ageGroup}` e.g. 25–29; `{period}` = ก.ย. 2568 – ส.ค. 2569. Label may wrap to 2 lines in half-card width (~160 px mobile). Also fixes the misspelling เปอร์เซนต์. |
