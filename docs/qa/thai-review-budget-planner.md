...
| BSP-074 ⚠️ | :562 | Label in the coloured box at the bottom of each card, above `{rate}%` | Average approval rate for Thai applicants · {sector}, applying from {location} · Home Affairs, {period} · not a prediction for you | เปอร์เซนต์ที่วีซ่าจะผ่าน | **Owner approved relabelling (2026-09-24).** Label: อัตราอนุมัติวีซ่าเฉลี่ยของผู้สมัครชาวไทย · Sub-line (new, 10 px): ข้อมูลกรมตรวจคนเข้าเมืองออสเตรเลีย {period} · ไม่ใช่การทำนายผลของคุณ | `{rate}` e.g. 94.6. `{period}` = ก.ย. 2568 – ส.ค. 2569 (from `GRANT_RATE_PERIOD`). Label ≤ 1 line in half-card width (~160 px mobile) — may wrap to 2. ⚠️ "เฉลี่ย" is only accurate if the age adjustment is removed; see `docs/qa/grant-rate-check.md` (open owner question). Also fixes the misspelling เปอร์เซนต์. |
